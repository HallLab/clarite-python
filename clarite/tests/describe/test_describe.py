...
def test_skewness(plantTraits, dropna):
    ...
    if dropna:
        correct_result = pd.DataFrame([
                ['pdias', 5.156840,  9.610278,  7.235317e-22],
                ['longindex', 0.163848,  0.742076,  4.580411e-01],
                ['durflow', 2.754286, 8.183515, 2.756827e-16],
                ['height', 0.583514, 2.735605, 6.226567e-03],
                ['begflow', -0.316648, -1.549449, 1.212738e-01],
            ], columns=["Variable", "skew", "zscore", "pvalue"])
    else:
        correct_result = pd.DataFrame([
                ['pdias', None, None, None],
                ['longindex', None, None, None],
                ['durflow', 2.754286, 8.183515, 2.756827e-16],
                ['height', 0.583514, 2.735605, 6.226567e-03],
                ['begflow', -0.316648, -1.549449, 1.212738e-01],
            ], columns=["Variable", "skew", "zscore", "pvalue"])
