...
def test_fpc_withoutfpc(data_fpc):
    ...
    # Make Design
    design = clarite.survey.SurveyDesignSpec(data_fpc, weights="weight", cluster="psuid", strata="stratid", nest=True)
...
def test_fpc_withfpc(data_fpc):
    ...
    # Make Design
    design = clarite.survey.SurveyDesignSpec(data_fpc, weights="weight", cluster="psuid", strata="stratid",
                                             fpc="Nh", nest=True)
    df = clarite.modify.colfilter(data_fpc, only=["x", "y"])
    # Get results
...
def test_fpc_withfpc_nostrata():
    ...
    # Get results
    loaded_result = load_surveylib_results(RESULT_PATH / "fpc_withfpc_nostrat_result.csv")
    python_result = clarite.analyze.ewas(phenotype="y", covariates=[], data=df, survey_design_spec=design, min_n=1)
    r_result = clarite.analyze.ewas(phenotype="y", covariates=[], data=df, survey_design_spec=design, min_n=1,
                                    regression_kind='r_survey')
    # Compare
...
def test_nhanes_noweights_withNA(data_NHANES_withNA):
    ...
    # Process data
    df = clarite.modify.colfilter(data_NHANES_withNA, only=["HI_CHOL", "RIAGENDR", "race", "agecat"])
    # Get Results
    loaded_result = load_surveylib_results(RESULT_PATH / "nhanes_noweights_withna_result.csv")
    python_result = pd.concat([
        clarite.analyze.ewas(phenotype="HI_CHOL", covariates=["agecat", "RIAGENDR"], data=df),
        clarite.analyze.ewas(phenotype="HI_CHOL", covariates=["race", "RIAGENDR"], data=df),
        clarite.analyze.ewas(phenotype="HI_CHOL", covariates=["race", "agecat"], data=df),
...
def test_nhanes_fulldesign(data_NHANES):
    ...
    # Make Design
    design = clarite.survey.SurveyDesignSpec(data_NHANES, weights="WTMEC2YR", cluster="SDMVPSU", strata="SDMVSTRA",
                                             fpc=None, nest=True)
    df = clarite.modify.colfilter(data_NHANES, only=["HI_CHOL", "RIAGENDR", "race", "agecat"])
    # Get Results
...
def test_nhanes_fulldesign_withna(data_NHANES_withNA):
    """Test the nhanes dataset with the full survey design"""
    # Make Design
    design = clarite.survey.SurveyDesignSpec(data_NHANES_withNA,
                                             weights="WTMEC2YR", cluster="SDMVPSU", strata="SDMVSTRA",
                                             fpc=None, nest=True)
    df = clarite.modify.colfilter(data_NHANES_withNA, only=["HI_CHOL", "RIAGENDR", "race", "agecat"])
    # Get Results
    loaded_result = load_surveylib_results(RESULT_PATH / "nhanes_complete_withna_result.csv")
    python_result = pd.concat([
        clarite.analyze.ewas(phenotype="HI_CHOL", covariates=["agecat", "RIAGENDR"], data=df,
                             survey_design_spec=design),
        clarite.analyze.ewas(phenotype="HI_CHOL", covariates=["race", "RIAGENDR"], data=df,
...
def test_nhanes_fulldesign_subset_category(data_NHANES):
    ...
    df = clarite.modify.colfilter(data_NHANES, only=["HI_CHOL", "RIAGENDR", "race", "agecat"])
    # Get Results
    loaded_result = load_surveylib_results(RESULT_PATH / "nhanes_complete_result_subset_cat.csv")
    python_result = pd.concat([
        clarite.analyze.ewas(phenotype="HI_CHOL", covariates=["agecat", "RIAGENDR"], data=df,
                             survey_design_spec=design),
        clarite.analyze.ewas(phenotype="HI_CHOL", covariates=["race", "RIAGENDR"], data=df,
...
def test_nhanes_fulldesign_subset_continuous():
    ...
    # Get Results
    loaded_result = load_surveylib_results(RESULT_PATH / "nhanes_complete_result_subset_cont.csv")
    python_result = pd.concat([
        clarite.analyze.ewas(phenotype="HI_CHOL", covariates=["agecat", "RIAGENDR"], data=df,
                             survey_design_spec=design),
        clarite.analyze.ewas(phenotype="HI_CHOL", covariates=["race", "RIAGENDR"], data=df,
...
def test_nhanes_weightsonly(data_NHANES):
    ...
    df = clarite.modify.colfilter(data_NHANES, only=["HI_CHOL", "RIAGENDR", "race", "agecat"])
    # Get Results
    loaded_result = load_surveylib_results(RESULT_PATH / "nhanes_weightsonly_result.csv")
    python_result = pd.concat([
        clarite.analyze.ewas(phenotype="HI_CHOL", covariates=["agecat", "RIAGENDR"], data=df,
                             survey_design_spec=design),
        clarite.analyze.ewas(phenotype="HI_CHOL", covariates=["race", "RIAGENDR"], data=df,
...
def test_nhanes_lonely_certainty(data_NHANES_lonely):
    ...
    # Make Design
    design = clarite.survey.SurveyDesignSpec(data_NHANES_lonely, weights="WTMEC2YR", cluster="SDMVPSU", strata="SDMVSTRA",
                                             fpc=None, nest=True, single_cluster='certainty')
    df = clarite.modify.colfilter(data_NHANES_lonely, only=["HI_CHOL", "RIAGENDR", "race", "agecat"])
    # Get Results
